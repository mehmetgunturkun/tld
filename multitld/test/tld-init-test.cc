...
int main(int argc, char** argv) {
    ...
    //trainForTesting(detector, firstFrame);
    printf(" ----- testing -----\n");
    ScoredBox* testBox = new ScoredBox(new Box(1, 138.135, 124.583, 228.135, 163.583));
    detector->nnClassifier->classify(secondFrame, testBox);
    NNScore* nnScore = (NNScore*) testBox->getScore("nn");
    printf("Conservative: %f\n", nnScore->conservativeScores[0]);
    return EXIT_SUCCESS;
}
